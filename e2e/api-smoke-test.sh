...
# Usage: BASE_URL=https://curious-dolphin-134.convex.site bash e2e/api-smoke-test.sh
set -euo pipefail

BASE="${BASE_URL:-${1:-}}"
if [ -z "$BASE" ]; then
  echo "ERROR: BASE_URL is required."
  echo "Usage: BASE_URL=https://curious-dolphin-134.convex.site bash e2e/api-smoke-test.sh"
  exit 1
fi
...
# Cleanup: delete all tasks created during this run, even on failure
cleanup() {
  if [ ${#CREATED_IDS[@]} -gt 0 ]; then
    ...
    for id in "${CREATED_IDS[@]}"; do
      ...
      HTTP=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$BASE/api/tasks/$id")
      if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
        echo "    ✅ Deleted"
      else
        # Fallback: set to cancelled
        HTTP2=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH "$BASE/api/tasks/$id" \
          -H "Content-Type: application/json" -d '{"status":"cancelled","notes":"E2E smoke test cleanup"}')
        echo "    ⚠️  DELETE returned $HTTP, PATCH cancel returned $HTTP2"
      fi
    done
  fi
}
...
if echo "$RESP" | jq -e '.meta.total >= 0 and (.meta | has("total") and has("lastUpdated")) and has("planning") and has("ready") and has("in_progress") and has("in_review") and has("done") and has("blocked")' &>/dev/null; then
  pass "Board endpoint"
else
...
if echo "$RESP" | jq -e '
  has("tasks") and has("total") and has("limit") and has("offset") and has("hasMore")
  and (.tasks[0] | has("title") and has("status") and has("priority") and has("_id") and has("_creationTime"))
' &>/dev/null; then
  pass "Tasks list"
else
...
ALL_DONE=$(echo "$RESP" | jq '[.tasks[].status] | all(. == "done")' 2>/dev/null || echo "false")
if [ "$COUNT" -ge 0 ] && [ "$COUNT" -le 5 ] && [ "$ALL_DONE" = "true" ]; then
  pass "Filter status=done&limit=5 (got $COUNT)"
else
  fail "Filter status=done&limit=5" "Count=$COUNT, AllDone=$ALL_DONE, Response: ${RESP:0:500}"
fi
...
if echo "$RESP" | jq -e '
  has("tasks") and has("total")
  and (.tasks | type == "array")
  and ((.tasks | length) == 0 or ([.tasks[] | (has("title") and has("status") and has("_id"))] | all))
' &>/dev/null; then
...
if [ -n "$CREATED_ID" ]; then
  ...
  if echo "$RESP" | jq -e '
    ...
    and .priority == "low"
    and has("_id") and has("status") and has("_creationTime") and has("createdBy")
  ' &>/dev/null; then
...
if [ -n "$CREATED_ID" ]; then
  ...
  HTTP=$(echo "$RESP" | tail -1)
  BODY=$(echo "$RESP" | sed '$d')
  if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ] && echo "$BODY" | jq -e '.success == true' &>/dev/null; then
...
if [ -n "$CREATED_ID" ]; then
  RESP=$(curl -sf "$BASE/api/tasks/$CREATED_ID" 2>&1) || RESP=""
  if echo "$RESP" | jq -e '.status == "ready" and .title == "E2E Smoke Test Task"' &>/dev/null; then
...
if echo "$RESP" | jq -e '
  ...
  and (to_entries[0].value | has("total") and has("byStatus") and has("byPriority"))
' &>/dev/null; then
...
RESP=$(curl -s -w "\n%{http_code}" "$BASE/api/tasks/invalid-id")
HTTP=$(echo "$RESP" | tail -1)
BODY=$(echo "$RESP" | sed '$d')
if ([ "$HTTP" -eq 400 ] || [ "$HTTP" -eq 404 ]) && echo "$BODY" | jq -e 'has("error") and (.error | type == "string") and (.error | length > 0)' &>/dev/null; then
  pass "Invalid ID returns $HTTP with error message"
else
  fail "Invalid ID" "Expected 400/404 with error object, got HTTP=$HTTP Body: ${BODY:0:500}"
fi
...
HTTP=$(echo "$RESP" | tail -1)
BODY=$(echo "$RESP" | sed '$d')
if [ "$HTTP" -eq 400 ] && echo "$BODY" | jq -e 'has("error") and (.error | type == "string") and (.error | test("title"; "i"))' &>/dev/null; then
  pass "Missing title returns 400 with title-related error"
else
  fail "Missing title" "Expected 400 with error mentioning title, got HTTP=$HTTP Body: ${BODY:0:500}"
fi

echo "12c. PATCH /api/tasks/nonexistent"
RESP=$(curl -s -w "\n%{http_code}" -X PATCH "$BASE/api/tasks/nonexistent-id-12345" \
  -H "Content-Type: application/json" -d '{"status":"ready"}')
HTTP=$(echo "$RESP" | tail -1)
BODY=$(echo "$RESP" | sed '$d')
if ([ "$HTTP" -eq 400 ] || [ "$HTTP" -eq 404 ]) && echo "$BODY" | jq -e 'has("error") and (.error | type == "string") and (.error | length > 0)' &>/dev/null; then
  pass "Nonexistent PATCH returns $HTTP with error message"
else
  fail "Nonexistent PATCH" "Expected 400/404 with error, got HTTP=$HTTP Body: ${BODY:0:500}"
fi

...
# Generate convex/_generated/api.ts stub when Convex codegen is absent
if [[ -f convex/_generated/api.d.ts || -f convex/_generated/api.ts ]]; then
  echo "⚠️  Skipping convex/_generated/api.ts stub generation because Convex generated API files already exist"
else
